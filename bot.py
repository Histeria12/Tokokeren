...
CHANNEL_TESTIMONI = os.environ.get("CHANNEL_TESTIMONI", "RanggaShoping")
ADMIN_USERNAME = "Galtzyyo"
SEABANK_REKENING = "901727395930"
SEABANK_NAMA = "CIT***"
BOT_USERNAME = os.environ.get("BOT_USERNAME", "wabadakstore_bot")

def sensor_teks(teks):
    """Sensor teks: tampilkan 2 karakter pertama + bintang + 1 karakter terakhir."""
    if not teks or len(teks) <= 2:
        return "***"
    if len(teks) <= 4:
        return teks[0] + "*" * (len(teks) - 1)
    return teks[:2] + "*" * (len(teks) - 3) + teks[-1]
...
PAKET = {
    ...
    "4": {"nama": "Meta One", "deskripsi": "WhatsApp Meta One Premium", "harga": 200000, "garansi": "Seumur Hidup"},
}
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("Meta One - Rp 200.000", callback_data="beli|4")],
...
async def foto_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if row:
        ...
        if not p:
            ...
            return
        nama_sensor = sensor_teks(user.first_name)
        username_sensor = sensor_teks(user.username) if user.username else "***"
        caption = (
            "BUKTI TRANSFER MASUK!\n\n"
            "Order ID: #" + str(order_id) + "\n"
            "Nama: " + nama_sensor + "\n"
            "Username: @" + username_sensor + "\n"
            "User ID: " + str(user.id) + "\n\n"
            "Paket: " + p["nama"] + "\n"
        ...
        # Simpan file_id foto untuk dipakai saat post testimoni ke channel
        context.application.bot_data["bukti_foto_" + str(order_id)] = photo
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "batal":
        ...
        teks = (
            ...
            "Transfer ke:\n"
            "Bank: SeaBank\n"
            "No Rekening: " + SEABANK_REKENING + "\n"
            "Atas Nama: " + SEABANK_NAMA + "\n"
            "Nominal: Rp " + f"{p['harga']:,}".replace(",", ".") + "\n\n"
            "Cara bayar:\n"
        ...
        # Ambil data order dari DB untuk testimoni
        conn = sqlite3.connect("toko.db")
        c = conn.cursor()
        c.execute("SELECT full_name, username, paket, harga FROM orders WHERE id=?", (order_id,))
        row = c.fetchone()
        conn.close()

        # Kirim notif ke buyer
        await context.bot.send_message(
            chat_id=uid,
        ...
        # Auto-post testimoni ke channel: foto bukti transfer + caption tersensor
        if row:
            nama_asli, username_asli, paket, harga = row
            nama_sensor = sensor_teks(nama_asli)
            username_sensor = sensor_teks(username_asli) if username_asli else "***"
            teks_testimoni = (
                "PEMBAYARAN BERHASIL!\n\n"
                "Nama: " + nama_sensor + "\n"
                "Username: @" + username_sensor + "\n\n"
                "Paket: " + paket + "\n"
                "Harga: Rp " + f"{harga:,}".replace(",", ".") + "\n\n"
                "Terima kasih sudah order di WA Badak Store!\n"
                "Order sekarang: t.me/" + BOT_USERNAME
            )
            try:
                # Ambil foto bukti transfer dari pesan admin (caption berisi order id)
                # Cari foto dari database via file_id yang disimpan
                foto_id = context.bot_data.get("bukti_foto_" + str(order_id))
                if foto_id:
                    await context.bot.send_photo(
                        chat_id="@" + CHANNEL_TESTIMONI,
                        photo=foto_id,
                        caption=teks_testimoni
                    )
                else:
                    await context.bot.send_message(
                        chat_id="@" + CHANNEL_TESTIMONI,
