...
async def create_qris_payment(amount, description, order_id):
    """Buat transaksi QRIS baru via Cutiezy API. Retry 3x kalau gagal."""
    def _call():
        ...
        for attempt in range(3):
            try:
                ...
                print(f"Cutiezy attempt {attempt+1} gagal: {e}")
                if attempt < 2:
                    time.sleep(2)
        return None
...
async def cek_status_payment(transaction_id):
    ...
    def _call():
        try:
            ...
            resp = req_sync.get(CUTIEZY_BASE_URL + "/check-payment/" + transaction_id, headers=headers, timeout=30)
            data = resp.json()
            if data.get("success"):
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "batal":
        ...
        img_bytes = await asyncio.get_event_loop().run_in_executor(
            None, lambda: req_sync.get(qris_image_url, timeout=30).content
        )
        await context.bot.send_photo(
            chat_id=user.id,
            photo=img_bytes,
